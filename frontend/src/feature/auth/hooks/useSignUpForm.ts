import { DM_signup } from "@services/dynasty-mommy/user"
...
export function useSignUpFrom() {
    ...
    const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        const response = await DM_signup(username, email, password)
        if (response) {
            //headers are response, if no headers then no authentication
            console.log(response)
        }
        else
            setError("Internal Service Error, Please Try Again")
